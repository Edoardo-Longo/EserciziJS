...
let monthDays = prompt('Inserire numero di giorni nel mese: ');
let weekDate = prompt('Inserire il primo giorno del mese: 0 Lunedì, 1 Martedì, 2 Mercoledì, 3 Giovedì, 4 Venerdì, 5 Sabato, 6 Domenica ');
let week = ["Lun", "Mar", "Mer", "Gio", "Ven", "Sab", "Dom"];

for (let i = 1, j = weekDate; i <= monthDays; i++, j++) {
  if (j >= week.length) {
    j = 0;
  }
  console.log(week[j], i);
}